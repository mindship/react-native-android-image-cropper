...
public class ImageCropperModule extends ReactContextBaseJavaModule implements ActivityEventListener {
  ...
  @ReactMethod
  public void selectImage(final String uri, final ReadableMap options, final Callback callback) {
    ...
    String GUIDELINES_KEY = "guideLines", TITLE_KEY = "title", CROPSHAPE_KEY = "cropShape",
        CROPMENU_BUTTON_TITLE_KEY = "cropMenuCropButtonTitle", REQUESTED_SIZE_HEIGHT_KEY = "requestedSizeHeight",
        REQUESTED_SIZE_WIDTH_KEY = "requestedSizeWidth", ALLOW_COUNTER_ROTATION_KEY = "allowCounterRotation",
        ALLOW_FLIPPING_KEY = "allowFlipping", ASPECT_RATIO_KEY = "aspectRatio", // array [x,y]
        AUTO_ZOOM_KEY = "autoZoomEnabled", MAX_ZOOM_KEY = "maxZoom", FIX_RATIO_KEY = "fixAspectRatio",
        CROP_WINDOW_PADDING_RATIO_KEY = "initialCropWindowPaddingRatio",
        BORDER_CORNER_THICKNESS_KEY = "borderCornerThickness", BORDER_CORNER_OFFSET_KEY = "borderCornerOffset",
        BORDER_CORNER_LENGTH_KEY = "borderCornerLength", GUIDELINES_THICKNESS_KEY = "guidelinesThickness",
        SNAP_RADIUS_KEY = "snapRadius", SHOW_CROP_OVERLAY_KEY = "showCropOverlay",
        MIN_CROP_WINDOW_KEY = "minCropWindowWidthHeight", FLIP_HORIZONTALLY_KEY = "flipHorizontally",
        FLIP_VERTICALLY_KEY = "flipVertically";

    CropImageView.Guidelines guidelines = ((CropImageView.Guidelines) GUIDELINES
        .get(options.getString(GUIDELINES_KEY))) != null
            ? (CropImageView.Guidelines) GUIDELINES.get(options.getString(GUIDELINES_KEY))
            : CropImageView.Guidelines.ON;
    CropImageView.CropShape cropShape = ((CropImageView.CropShape) CROPSHAPES
        .get(options.getString(CROPSHAPE_KEY))) != null
            ? (CropImageView.CropShape) CROPSHAPES.get(options.getString(CROPSHAPE_KEY))
            : CropImageView.CropShape.RECTANGLE;

    CropImage.activity(Uri.parse(uri)).setGuidelines(guidelines).setActivityTitle(options.getString(TITLE_KEY))
        .setCropShape(cropShape).setCropMenuCropButtonTitle(options.getString(CROPMENU_BUTTON_TITLE_KEY))
        .setRequestedSize(options.getInt(REQUESTED_SIZE_HEIGHT_KEY), options.getInt(REQUESTED_SIZE_WIDTH_KEY))
        .setAllowCounterRotation(options.getBoolean(ALLOW_COUNTER_ROTATION_KEY))
        .setAllowFlipping(options.getBoolean(ALLOW_FLIPPING_KEY)).setInitialCropWindowPaddingRatio(0)
        .setAspectRatio(
        ...
        .setAutoZoomEnabled(options.getBoolean(AUTO_ZOOM_KEY)).setMaxZoom(options.getInt(MAX_ZOOM_KEY))
        ...
        .setFlipVertically(options.getBoolean(FLIP_VERTICALLY_KEY)).start(getCurrentActivity());
